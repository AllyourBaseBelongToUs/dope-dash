"""Claude Code agent wrapper for Dope Dash event streaming.

This wrapper monitors Claude Code agent execution by intercepting
stdout/stderr from the Claude process, captures events, transforms them
to Dope Dash format, and streams them to the WebSocket server.

Features:
- Intercepts Claude Code process stdout/stderr
- Parses Claude-specific events (tool_use, response, error)
- Transforms Claude events to unified event format
- Detects Claude sessions via process scanning
- Sends events to WebSocket server via HTTP POST
- Handles process crashes and restarts
- Polling fallback when WebSocket unavailable
- Adds Claude-specific metadata (model, tools used)
- Creates and manages Claude sessions in database

Usage:
    python -m wrappers.claude_wrapper
    python -m wrappers.claude_wrapper --project-dir /path/to/project
    python -m wrappers.claude_wrapper --pid 12345
"""
from __future__ import annotations

import argparse
import asyncio
import json
import logging
import os
import re
import signal
import sys
import uuid
from dataclasses import dataclass, field
from datetime import datetime, timezone
from enum import Enum
from pathlib import Path
from typing import Any

import aiohttp
import psutil
from pydantic import BaseModel, ValidationError

# Add parent directory to path for imports
sys.path.insert(0, str(Path(__file__).parent.parent))

from app.core.config import settings
from app.models.session import Session, SessionStatus, AgentType
from app.models.unified_events import (
    UnifiedEventFactory,
    TaskEvent,
    ErrorEvent,
    MessageEvent,
    CommandEvent,
)
from db.connection import db_manager, get_db_session


logger = logging.getLogger(__name__)

# Default configuration
DEFAULT_WEBSOCKET_URL = "http://localhost:8001"
DEFAULT_PROJECT_DIR = Path.cwd()

# Claude-specific directories
CLAUDE_CONFIG_DIR = Path.home() / ".claude"
CLAUDE_SESSIONS_DIR = CLAUDE_CONFIG_DIR / "sessions"
CLAUDE_LOGS_DIR = CLAUDE_CONFIG_DIR / "logs"

# Polling intervals (seconds)
POLLING_INTERVAL = 5
HEARTBEAT_CHECK_INTERVAL = 2
PROCESS_CHECK_INTERVAL = 1


class ClaudeEventType(str, Enum):
    """Claude-specific event types parsed from stdout/stderr."""

    # Tool invocation events
    TOOL_USE_START = "tool_use_start"
    TOOL_USE_COMPLETE = "tool_use_complete"
    TOOL_USE_ERROR = "tool_use_error"

    # Response events
    RESPONSE_START = "response_start"
    RESPONSE_COMPLETE = "response_complete"
    RESPONSE_STREAM = "response_stream"

    # Error events
    ERROR = "error"
    WARNING = "warning"

    # Session events
    SESSION_START = "session_start"
    SESSION_END = "session_end"
    HEARTBEAT = "heartbeat"

    # File operations
    FILE_READ = "file_read"
    FILE_WRITE = "file_write"
    FILE_EDIT = "file_edit"

    # Command execution
    COMMAND_START = "command_start"
    COMMAND_COMPLETE = "command_complete"


class ClaudeTool(str, Enum):
    """Claude Code tools that can be invoked."""

    BASH = "Bash"
    READ = "Read"
    WRITE = "Write"
    EDIT = "Edit"
    GREP = "Grep"
    GLOB = "Glob"
    TASK = "Task"
    WEB_SEARCH = "WebSearch"
    EXIT_PLAN_MODE = "ExitPlanMode"
    ENTER_PLAN_MODE = "EnterPlanMode"
    TODO_WRITE = "TodoWrite"
    ASK_USER_QUESTION = "AskUserQuestion"


@dataclass
class ClaudeEvent:
    """Parsed Claude event from stdout/stderr."""

    event_type: ClaudeEventType
    timestamp: str
    raw: dict = field(default_factory=dict)

    # Tool-specific fields
    tool_name: str | None = None
    tool_input: dict | None = None
    tool_output: str | None = None

    # Response-specific fields
    role: str | None = None
    content: str | None = None

    # Error-specific fields
    error_message: str | None = None
    error_type: str | None = None

    # Command-specific fields
    command: str | None = None
    args: list[str] | None = None
    exit_code: int | None = None

    @classmethod
    def from_dict(cls, data: dict) -> ClaudeEvent:
        """Create ClaudeEvent from dictionary."""
        return cls(
            event_type=ClaudeEventType(data.get("event_type", "unknown")),
            timestamp=data.get("timestamp", datetime.now(timezone.utc).isoformat()),
            raw=data,
            tool_name=data.get("tool_name"),
            tool_input=data.get("tool_input"),
            tool_output=data.get("tool_output"),
            role=data.get("role"),
            content=data.get("content"),
            error_message=data.get("error_message"),
            error_type=data.get("error_type"),
            command=data.get("command"),
            args=data.get("args"),
            exit_code=data.get("exit_code"),
        )


@dataclass
class ClaudeSessionInfo:
    """Information about a Claude session."""

    project_name: str
    session_id: uuid.UUID
    pid: int | None = None
    working_dir: str | None = None
    command: str | None = None
    started_at: datetime | None = None
    status: SessionStatus = SessionStatus.RUNNING
    model: str | None = None  # Claude model being used


class EventIngestRequest(BaseModel):
    """Request schema for event ingestion."""

    session_id: uuid.UUID
    event_type: str
    data: dict = {}


class ClaudeWrapper:
    """Claude Code agent wrapper for event streaming.

    This class monitors Claude agent execution and streams events to Dope Dash.
    """

    def __init__(
        self,
        project_dir: Path,
        websocket_url: str = DEFAULT_WEBSOCKET_URL,
        pid: int | None = None,
        working_dir: str | None = None,
        command: str | None = None,
    ) -> None:
        """Initialize the Claude wrapper.

        Args:
            project_dir: Path to the project directory
            websocket_url: URL of the WebSocket server for event ingestion
            pid: Process ID of the Claude agent (optional)
            working_dir: Working directory of the agent (optional)
            command: Command line used to start the agent (optional)
        """
        self.project_dir = project_dir.resolve()
        self.websocket_url = websocket_url.rstrip("/")
        self.project_name = self.project_dir.name

        # Agent metadata
        self._pid = pid
        self._working_dir = working_dir or str(self.project_dir)
        self._command = command

        # Session management
        self.session_info: ClaudeSessionInfo | None = None
        self._running = False
        self._http_session: aiohttp.ClientSession | None = None

        # Process monitoring
        self._process_task: asyncio.Task | None = None
        self._stdout_reader: asyncio.Task | None = None
        self._stderr_reader: asyncio.Task | None = None

        # Event queue
        self._event_queue: asyncio.Queue[ClaudeEvent] = asyncio.Queue()

        # Polling fallback
        self._polling_mode = False
        self._queued_events: list[EventIngestRequest] = []

        logger.info(
            f"Claude wrapper initialized for project: {self.project_name} "
            f"at {self.project_dir}"
        )

    async def start(self) -> None:
        """Start the Claude wrapper event monitoring loop."""
        if self._running:
            logger.warning("Claude wrapper already running")
            return

        self._running = True
        self._http_session = aiohttp.ClientSession()

        logger.info("Starting Claude wrapper event monitoring...")

        # Initialize database
        db_manager.init_db()

        try:
            # Detect or create Claude session
            await self._ensure_session()

            # Start monitoring loop
            await self._monitoring_loop()

        except asyncio.CancelledError:
            logger.info("Claude wrapper monitoring cancelled")
        except Exception as e:
            logger.error(f"Error in Claude wrapper: {e}", exc_info=True)
        finally:
            await self.stop()

    async def stop(self) -> None:
        """Stop the Claude wrapper and cleanup resources."""
        if not self._running:
            return

        logger.info("Stopping Claude wrapper...")
        self._running = False

        # Cancel monitoring tasks
        for task in [self._process_task, self._stdout_reader, self._stderr_reader]:
            if task and not task.done():
                task.cancel()
                try:
                    await task
                except asyncio.CancelledError:
                    pass

        # End session
        if self.session_info and self.session_info.status == SessionStatus.RUNNING:
            await self._end_session(SessionStatus.COMPLETED)

        # Flush queued events
        if self._queued_events:
            logger.info(f"Flushing {len(self._queued_events)} queued events")
            for event in self._queued_events:
                await self._send_to_websocket(event)

        # Close HTTP session
        if self._http_session:
            await self._http_session.close()
            self._http_session = None

        logger.info("Claude wrapper stopped")

    async def _ensure_session(self) -> None:
        """Ensure a Claude session exists for this project."""
        # Detect active Claude session
        detected_session = await self._detect_claude_session()

        if detected_session:
            self.session_info = detected_session
            logger.info(f"Detected active Claude session: {detected_session.session_id}")
        else:
            # Create new session
            await self._create_session()

    async def _create_session(self) -> None:
        """Create a new Claude session in the database."""
        # Detect Claude process info
        pid = await self._check_claude_process()
        command = self._command
        working_dir = self._working_dir

        # Try to get command from process
        if pid and not command:
            try:
                proc = psutil.Process(pid)
                command = " ".join(proc.cmdline()) if proc.cmdline() else None
                working_dir = proc.cwd() if proc.cwd() else working_dir
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                pass

        # Try to detect Claude model from command
        model = self._extract_model_from_command(command) if command else None

        async with get_db_session() as db_session:
            db_session_obj = Session(
                agent_type=AgentType.CLAUDE,
                project_name=self.project_name,
                status=SessionStatus.RUNNING,
                metadata={
                    "wrapper": "claude_wrapper",
                    "project_dir": str(self.project_dir),
                    "model": model,
                },
                started_at=datetime.now(timezone.utc),
                pid=pid,
                working_dir=working_dir,
                command=command,
            )
            db_session.add(db_session_obj)
            await db_session.flush()
            await db_session.refresh(db_session_obj)

            self.session_info = ClaudeSessionInfo(
                project_name=self.project_name,
                session_id=db_session_obj.id,
                pid=pid,
                working_dir=working_dir,
                command=command,
                started_at=datetime.now(timezone.utc),
                status=SessionStatus.RUNNING,
                model=model,
            )

            logger.info(f"Created new Claude session: {db_session_obj.id}")

            # Emit session start event
            await self._emit_event(
                *UnifiedEventFactory.session_start(
                    project_name=self.project_name,
                    agent_type="claude",
                    metadata={"model": model},
                ),
            )

    def _extract_model_from_command(self, command: str | None) -> str | None:
        """Extract Claude model from command line.

        Args:
            command: Command line string

        Returns:
            Model name if detected, None otherwise
        """
        if not command:
            return None

        # Look for model patterns like --model opus, --model sonnet, etc.
        model_patterns = [
            r"--model\s+(\w+)",
            r"-m\s+(\w+)",
            r"model[=:](\w+)",
        ]

        for pattern in model_patterns:
            match = re.search(pattern, command)
            if match:
                return match.group(1)

        return None

    async def _end_session(self, status: SessionStatus) -> None:
        """End the current Claude session.

        Args:
            status: Final status of the session
        """
        if not self.session_info:
            return

        async with get_db_session() as db_session:
            from sqlalchemy import select

            result = await db_session.execute(
                select(Session).where(Session.id == self.session_info.session_id)
            )
            db_session_obj = result.scalar_one_or_none()

            if db_session_obj:
                db_session_obj.status = status
                db_session_obj.ended_at = datetime.now(timezone.utc)
                await db_session.commit()

        self.session_info.status = status

        logger.info(f"Claude session {self.session_info.session_id} ended with status: {status}")

    async def _detect_claude_session(self) -> ClaudeSessionInfo | None:
        """Detect active Claude session via process scan.

        Returns:
            ClaudeSessionInfo if active session detected, None otherwise
        """
        # Check for Claude processes
        pid = await self._check_claude_process()
        if pid:
            # Try to find existing session in database
            async with get_db_session() as db_session:
                from sqlalchemy import select

                result = await db_session.execute(
                    select(Session)
                    .where(Session.project_name == self.project_name)
                    .where(Session.agent_type == AgentType.CLAUDE)
                    .where(Session.status == SessionStatus.RUNNING)
                    .order_by(Session.started_at.desc())
                    .limit(1)
                )
                db_session_obj = result.scalar_one_or_none()

                if db_session_obj:
                    # Get process info
                    try:
                        proc = psutil.Process(pid)
                        command = " ".join(proc.cmdline()) if proc.cmdline() else None
                        working_dir = proc.cwd() if proc.cwd() else None
                        model = self._extract_model_from_command(command)
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        command = None
                        working_dir = None
                        model = None

                    return ClaudeSessionInfo(
                        project_name=self.project_name,
                        session_id=db_session_obj.id,
                        pid=pid,
                        working_dir=working_dir,
                        command=command,
                        started_at=db_session_obj.started_at.replace(tzinfo=timezone.utc),
                        status=SessionStatus.RUNNING,
                        model=model,
                    )

        return None

    async def _check_claude_process(self) -> int | None:
        """Check for running Claude Code processes.

        Returns:
            PID if found, None otherwise
        """
        try:
            for proc in psutil.process_iter(["pid", "name", "cmdline", "cwd"]):
                try:
                    cmdline = proc.info.get("cmdline", [])
                    if cmdline:
                        cmdline_str = " ".join(cmdline)
                        cwd = proc.info.get("cwd", "")

                        # Check for claude commands
                        # Match patterns like:
                        # - "claude" in command
                        # - Working directory matches project
                        is_claude = (
                            "claude" in cmdline_str.lower()
                            and not any(
                                x in cmdline_str.lower()
                                for x in ["claude-code", "anx-kt"]
                            )  # Exclude editor processes
                            and (
                                str(self.project_dir) in cwd
                                or self._working_dir in cwd
                                or cwd == str(self.project_dir)
                            )
                        )

                        if is_claude:
                            return proc.info["pid"]

                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    continue

        except Exception:
            pass

        return None

    async def _monitoring_loop(self) -> None:
        """Main monitoring loop for Claude process and events."""
        logger.info("Starting Claude monitoring loop...")

        # Start event processor
        event_processor = asyncio.create_task(self._event_processor())

        while self._running:
            try:
                # Check for Claude process
                pid = await self._check_claude_process()

                if not pid:
                    logger.debug("No Claude process detected, waiting...")
                    await asyncio.sleep(PROCESS_CHECK_INTERVAL)
                    continue

                # Update PID if changed
                if pid != self._pid:
                    self._pid = pid
                    logger.info(f"Detected Claude process: {pid}")

                    # Update session heartbeat
                    await self._update_heartbeat()

                # Try to attach to process for stdout/stderr interception
                if not self._process_task or self._process_task.done():
                    self._process_task = asyncio.create_task(
                        self._monitor_process_output(pid)
                    )

                # Poll for events if interception failed
                if self._polling_mode:
                    await self._poll_for_events()

                await asyncio.sleep(HEARTBEAT_CHECK_INTERVAL)

            except asyncio.CancelledError:
                raise
            except Exception as e:
                logger.error(f"Error in monitoring loop: {e}", exc_info=True)
                await asyncio.sleep(POLLING_INTERVAL)

        # Cleanup event processor
        event_processor.cancel()
        try:
            await event_processor
        except asyncio.CancelledError:
            pass

    async def _monitor_process_output(self, pid: int) -> None:
        """Monitor Claude process output for events.

        Args:
            pid: Process ID to monitor
        """
        try:
            proc = psutil.Process(pid)

            # Try to open stdout/stderr for reading
            # Note: This requires the process to be started with appropriate flags
            # or may not work for all processes

            # For now, use polling fallback
            self._polling_mode = True
            logger.debug(f"Using polling mode for Claude process {pid}")

        except (psutil.NoSuchProcess, psutil.AccessDenied) as e:
            logger.warning(f"Cannot monitor process {pid}: {e}")
            self._polling_mode = True

    async def _poll_for_events(self) -> None:
        """Poll for Claude events from log files or other sources.

        This is the fallback method when direct stdout/stderr interception
        is not available.
        """
        # Check Claude logs directory
        if CLAUDE_LOGS_DIR.exists():
            # Look for recent log files
            log_files = list(CLAUDE_LOGS_DIR.glob("*.log"))
            log_files.sort(key=lambda p: p.stat().st_mtime, reverse=True)

            for log_file in log_files[:5]:  # Check last 5 logs
                try:
                    # Read new lines from log file
                    async with aiohttp.ClientSession() as session:
                        async with session.get(f"file://{log_file}") as response:
                            # Process log lines for events
                            pass
                except Exception:
                    pass

        # Check for session files
        if CLAUDE_SESSIONS_DIR.exists():
            session_files = list(CLAUDE_SESSIONS_DIR.glob("*.json"))
            for session_file in session_files:
                try:
                    data = json.loads(session_file.read_text())
                    # Parse session data for events
                    await self._parse_session_data(data)
                except (json.JSONDecodeError, IOError):
                    pass

    async def _parse_session_data(self, data: dict) -> None:
        """Parse Claude session data for events.

        Args:
            data: Session data dictionary
        """
        # Look for tool use events
        if "tool_calls" in data:
            for tool_call in data["tool_calls"]:
                event = ClaudeEvent(
                    event_type=ClaudeEventType.TOOL_USE_START,
                    timestamp=datetime.now(timezone.utc).isoformat(),
                    tool_name=tool_call.get("name"),
                    tool_input=tool_call.get("input"),
                )
                await self._event_queue.put(event)

        # Look for response events
        if "messages" in data:
            for message in data["messages"]:
                if message.get("role") == "assistant":
                    event = ClaudeEvent(
                        event_type=ClaudeEventType.RESPONSE_COMPLETE,
                        timestamp=datetime.now(timezone.utc).isoformat(),
                        role="assistant",
                        content=message.get("content"),
                    )
                    await self._event_queue.put(event)

    async def _event_processor(self) -> None:
        """Process events from the queue and send to WebSocket."""
        while self._running:
            try:
                event = await asyncio.wait_for(
                    self._event_queue.get(),
                    timeout=1.0,
                )

                # Transform to unified event
                unified_event = self._transform_event(event)

                # Emit event
                await self._emit_event(*unified_event)

            except asyncio.TimeoutError:
                continue
            except asyncio.CancelledError:
                raise
            except Exception as e:
                logger.error(f"Error processing event: {e}", exc_info=True)

    def _transform_event(
        self, event: ClaudeEvent
    ) -> tuple[str, dict[str, Any]]:
        """Transform Claude event to unified event format.

        Args:
            event: Claude event to transform

        Returns:
            Tuple of (event_type, data)
        """
        # Tool use events
        if event.event_type == ClaudeEventType.TOOL_USE_START:
            task_event = TaskEvent(
                task_id=f"tool_{event.tool_name}_{uuid.uuid4().hex[:8]}",
                task_type="tool_use",
                task_name=event.tool_name or "unknown",
                metadata={"tool_input": event.tool_input},
            )
            return UnifiedEventFactory.task_start(task_event)

        if event.event_type == ClaudeEventType.TOOL_USE_COMPLETE:
            return UnifiedEventFactory.task_complete(
                task_id=f"tool_{event.tool_name}",
                result={"output": event.tool_output},
            )

        if event.event_type == ClaudeEventType.TOOL_USE_ERROR:
            return UnifiedEventFactory.task_fail(
                task_id=f"tool_{event.tool_name}",
                error=event.error_message or "Unknown error",
                error_type=event.error_type,
            )

        # Response events
        if event.event_type in (
            ClaudeEventType.RESPONSE_START,
            ClaudeEventType.RESPONSE_COMPLETE,
        ):
            message_event = MessageEvent(
                role=event.role or "assistant",
                content=event.content or "",
                metadata={"timestamp": event.timestamp},
            )
            return UnifiedEventFactory.message(message_event)

        # Error events
        if event.event_type == ClaudeEventType.ERROR:
            error_event = ErrorEvent(
                message=event.error_message or "Unknown error",
                error_type=event.error_type or "ClaudeError",
            )
            return UnifiedEventFactory.error(error_event)

        # Command events
        if event.event_type == ClaudeEventType.COMMAND_START:
            command_event = CommandEvent(
                command=event.command or "",
                args=event.args or [],
            )
            return UnifiedEventFactory.command(command_event)

        if event.event_type == ClaudeEventType.COMMAND_COMPLETE:
            command_event = CommandEvent(
                command=event.command or "",
                args=event.args or [],
                exit_code=event.exit_code,
            )
            return UnifiedEventFactory.command(command_event)

        # Default: pass through as-is
        return event.event_type.value, event.raw

    async def _emit_event(
        self,
        event_type: str,
        data: dict[str, Any],
    ) -> None:
        """Emit an event to the WebSocket server.

        Args:
            event_type: Type of event to emit
            data: Event data payload
        """
        if not self.session_info:
            logger.warning("No session info, skipping event emission")
            return

        event = EventIngestRequest(
            session_id=self.session_info.session_id,
            event_type=event_type,
            data=data,
        )

        # Try to send via WebSocket API
        success = await self._send_to_websocket(event)

        # Fallback to polling if WebSocket unavailable
        if not success:
            logger.debug("WebSocket unavailable, event queued for polling")
            self._queued_events.append(event)

    async def _send_to_websocket(self, event: EventIngestRequest) -> bool:
        """Send event to WebSocket server via HTTP POST.

        Args:
            event: Event to send

        Returns:
            True if successful, False otherwise
        """
        if not self._http_session:
            return False

        url = f"{self.websocket_url}/api/events"

        try:
            async with self._http_session.post(
                url,
                json=event.model_dump(),
                headers={"Content-Type": "application/json"},
                timeout=aiohttp.ClientTimeout(total=5),
            ) as response:
                if response.status == 200:
                    logger.debug(f"Event {event.event_type} sent successfully")
                    return True
                else:
                    logger.warning(f"WebSocket server returned status {response.status}")
                    return False

        except (aiohttp.ClientError, asyncio.TimeoutError):
            logger.debug(f"Failed to send event to WebSocket server")
            return False

    async def _update_heartbeat(self) -> None:
        """Update the heartbeat timestamp for the current session."""
        if not self.session_info:
            return

        async with get_db_session() as db_session:
            from sqlalchemy import select

            result = await db_session.execute(
                select(Session).where(Session.id == self.session_info.session_id)
            )
            db_session_obj = result.scalar_one_or_none()

            if db_session_obj:
                db_session_obj.last_heartbeat = datetime.now(timezone.utc)
                await db_session.commit()


async def run_wrapper(
    project_dir: Path,
    websocket_url: str,
    pid: int | None = None,
    working_dir: str | None = None,
    command: str | None = None,
) -> None:
    """Run the Claude wrapper with signal handling.

    Args:
        project_dir: Project directory path
        websocket_url: WebSocket server URL
        pid: Process ID of Claude agent
        working_dir: Working directory
        command: Command line
    """
    wrapper = ClaudeWrapper(
        project_dir=project_dir,
        websocket_url=websocket_url,
        pid=pid,
        working_dir=working_dir,
        command=command,
    )

    # Setup signal handlers
    loop = asyncio.get_running_loop()

    def signal_handler() -> None:
        logger.info("Received shutdown signal")
        asyncio.create_task(wrapper.stop())

    for sig in (signal.SIGTERM, signal.SIGINT):
        loop.add_signal_handler(sig, signal_handler)

    try:
        await wrapper.start()
    except asyncio.CancelledError:
        logger.info("Wrapper cancelled")
    finally:
        # Remove signal handlers
        for sig in (signal.SIGTERM, signal.SIGINT):
            loop.remove_signal_handler(sig)


def main() -> None:
    """Main entry point for the Claude wrapper."""
    parser = argparse.ArgumentParser(
        description="Claude Code agent wrapper for Dope Dash",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m wrappers.claude_wrapper
  python -m wrappers.claude_wrapper --project-dir /path/to/project
  python -m wrappers.claude_wrapper --pid 12345
  python -m wrappers.claude_wrapper --websocket-url http://localhost:8001
  python -m wrappers.claude_wrapper --debug
        """,
    )

    parser.add_argument(
        "--project-dir",
        type=Path,
        default=DEFAULT_PROJECT_DIR,
        help="Path to project directory (default: current directory)",
    )

    parser.add_argument(
        "--websocket-url",
        type=str,
        default=DEFAULT_WEBSOCKET_URL,
        help=f"WebSocket server URL (default: {DEFAULT_WEBSOCKET_URL})",
    )

    parser.add_argument(
        "--pid",
        type=int,
        default=None,
        help="Process ID of Claude agent (optional, auto-detected if not provided)",
    )

    parser.add_argument(
        "--working-dir",
        type=str,
        default=None,
        help="Working directory of Claude agent (optional)",
    )

    parser.add_argument(
        "--command",
        type=str,
        default=None,
        help="Command line used to start Claude (optional)",
    )

    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug logging",
    )

    args = parser.parse_args()

    # Setup logging
    log_level = logging.DEBUG if args.debug else logging.INFO
    logging.basicConfig(
        level=log_level,
        format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )

    # Validate project directory
    if not args.project_dir.exists():
        logger.error(f"Project directory does not exist: {args.project_dir}")
        sys.exit(1)

    # Run the wrapper
    try:
        asyncio.run(
            run_wrapper(
                args.project_dir,
                args.websocket_url,
                args.pid,
                args.working_dir,
                args.command,
            )
        )
    except KeyboardInterrupt:
        logger.info("Wrapper interrupted by user")
    except Exception as e:
        logger.error(f"Wrapper error: {e}", exc_info=True)
        sys.exit(1)


if __name__ == "__main__":
    main()
